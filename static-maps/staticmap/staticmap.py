...
class TextMarker():
    def __init__(self, coord, color, content, font_size):
        """
        :param coord: a lon-lat pair, eg (175.0, 0.0)
        :type coord: tuple
        :param color: color suitable for PIL / Pillow
        ...
        self.font = ImageFont.truetype(os.path.dirname(
            os.path.abspath(__file__)) + '/fonts/Poppins-Medium.ttf', font_size)

    @property
    def extent_px(self):
        w, h = self.font.getsize(self.content)
        return (0,h,w,0,)
...
class StaticMap:
    ...
    def _calculate_zoom(self):
        ...
        for z in range(17, -1, -1):
            ...
            width = (_lon_to_x(extent[2], z) -
                     _lon_to_x(extent[0], z)) * self.tile_size
            if width > (self.width - self.padding[0] * 2):
                continue

            height = (_lat_to_y(extent[1], z) -
                      _lat_to_y(extent[3], z)) * self.tile_size
        ...
        for nb_retry in itertools.count():
            ...
            for tile, future in zip(tiles, futures):
                ...
                tile_image = Image.open(
                    BytesIO(response_content)).convert("RGBA")
                box = [
                    self._x_to_px(x),
                    self._y_to_px(y),
                    self._x_to_px(x + 1),
    ...
    def _draw_features(self, image):
        ...
        for polygon in self.polygons:
            ...
            if polygon.fill_color or polygon.outline_color:
                draw.polygon(points, fill=polygon.fill_color,
        ...
        for lable in filter(lambda m: isinstance(m, TextMarker), self.markers):
            point = [
                self._x_to_px(_lon_to_x(lable.coord[0], self.zoom)) * 2,
                self._y_to_px(_lat_to_y(lable.coord[1], self.zoom)) * 2
            ]

            draw.text(point, lable.content, lable.color, anchor="mm",
                      font=lable.font, stroke_width=1, stroke_fill='#ffffff')
        ...
        for icon in filter(lambda m: isinstance(m, IconMarker), self.markers):
            position = (
                self._x_to_px(
                    _lon_to_x(icon.coord[0], self.zoom)) - icon.offset[0],
                self._y_to_px(
                    _lat_to_y(icon.coord[1], self.zoom)) - icon.offset[1]
            )
...
if __name__ == '__main__':
    ...
    marker = TextMarker([2.3, 48.9], '#0E8BDE', 'long test text')
